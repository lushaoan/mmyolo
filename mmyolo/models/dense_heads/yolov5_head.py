...
def get_prior_xy_info(
    index: int, num_base_priors: int, featmap_sizes: int
) -> Tuple[int, int, int]:
...
class YOLOv5HeadModule(BaseModule):
    ...
    def __init__(
        ...
        if isinstance(in_channels, int):
            self.in_channels = [
                make_divisible(in_channels, widen_factor)
            ] * self.num_levels
        else:
    ...
    def _init_layers(self):
        ...
        for i in range(self.num_levels):
            conv_pred = nn.Conv2d(
                self.in_channels[i], self.num_base_priors * self.num_out_attrib, 1
            )
...
class YOLOv5Head(BaseDenseHead):
    ...
    def forward(self, x: Tuple[Tensor]) -> Tuple[List]:
        ...
        Returns:
            ...
            predictions, and objectnesses.
        """
        # print("YOLOv5Head.forward head_module", self.head_module)
    ...
    def predict_by_feat(
        ...
        # print("yolo v5 head predict_by_feat")
        ...
        # print("self.mlvl_priors")
        # for ele in self.mlvl_priors:
        #     print(ele.shape)
        # print(self.mlvl_priors[0])

        flatten_priors = torch.cat(self.mlvl_priors)
        # print("flatten_priors shape", flatten_priors.shape)
        ...
        # print("flatten stride size", flatten_stride.shape)
        # print("flatten_stride shape", flatten_stride.shape)
        ...
        # for bbox in flatten_bbox_preds:
        #     print(bbox.shape)
